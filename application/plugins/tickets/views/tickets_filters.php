  ...
  <p id="categoryFilters">
    ...
    if (is_array($categories)) {
      foreach ($categories as $category) {
        ...
        echo '<a class="btn btn-default btn-xs" href="'.get_url('tickets', 'index', array_merge($params, array($property_name => $category_id))).'" '.(preg_match("/^(.*,)?$category_id(,.*)?$/", $property_in_url) ? 'class="selected"' : '').'>'.$category->getName().'</a> ';
        if (preg_match("/^(.*,)?$category_id(,.*)?$/", $property_in_url)) {
          echo '<a href="'.get_url('tickets', 'index', array_merge($params, array($property_name => preg_replace(array("/^$category_id,/", "/,$category_id,/", "/,$category_id$/","/^$category_id$/"), array('', ',', '', ''), $property_in_url)))).'"><i class="icon icon-minus-sign"></i></a> ';
        } else {
          echo '<a href="'.get_url('tickets', 'index', array_merge($params, array($property_name => ($property_in_url == "" ? $category_id : $property_in_url.','.$category_id)))).'"><i class="icon icon-plus-sign"></i></a> ';
        } // if
      } // foreach
    } // foreach
    ?>
  </p>
  <p id="assignedToFilters">
    ...
    foreach ($grouped_users as $company_id => $company_users) {
      ...
      foreach ($company_users as $user) {
        $user_id = $user->getId();
        echo '<a class="btn btn-default btn-xs" href="'.get_url('tickets', 'index', array_merge($params, array($property_name => $user_id))).'" '.(preg_match("/^(.*,)?$user_id(,.*)?$/", $property_in_url) ? 'class="selected"' : '').'>'.$user->getDisplayName().'</a> ';
        if (preg_match("/^(.*,)?$user_id(,.*)?$/", $property_in_url)) {
          echo '<a href="'.get_url('tickets', 'index', array_merge($params, array($property_name => preg_replace(array("/^$user_id,/", "/,$user_id,/", "/,$user_id$/","/^$user_id$/"), array('', ',', '', ''), $property_in_url)))).'"><i class="icon icon-minus-sign"></i></a> ';
        } else {
          echo '<a href="'.get_url('tickets', 'index', array_merge($params, array($property_name => ($property_in_url == "" ? $user_id : $property_in_url.','.$user_id)))).'"><i class="icon icon-plus-sign"></i></a> ';
        } // if
      } // foreach
    } // foreach
    ?>
  </p>
  <p id="createdByFilters">
