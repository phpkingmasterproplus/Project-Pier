...
<?php if ($company->countUsers() > 0) { ?>
  <div class="projectCompany">
    <div class="projectCompanyLogo">
      <img src="<?php echo $company->getLogoUrl() ?>" alt="<?php echo clean($company->getName()) ?>" />
    </div>
    <div class="projectCompanyMeta block">
      <div class="header">
      <?php if ($company->isOwner()) { ?>
        <h1><?php echo clean($company->getName()) ?></h1>
        <input type="hidden" name="project_company_<?php echo $company->getId() ?>" value="checked" />
      <?php } else { ?>
        <?php echo checkbox_field('project_company_' . $company->getId(), $company->isProjectCompany(active_project()), array('id' => 'project_company_' . $company->getId() )) ?>
        <label for="<?php echo 'project_company_' . $company->getId() ?>" class="checkbox"><?php echo clean($company->getName()) ?></label>
      <?php } // if ?>
      </div>
        ...
        <?php foreach ($users as $user) { ?>
          <tr>
            ...
            <?php if ($user->isAdministrator() || ($user->getId() == active_project()->getCreatedById()) ) { ?>
              ...
              <a href="<?php echo $user->getUpdatePermissionsUrl() ?>"><?php echo clean($user->getDisplayName()) ?></a>
            ...
            <?php if ($company->isOwner()) { ?>
              <i class="icon icon-ok"></i>
              <strong><?php echo lang('all permissions') ?><strong>
            <?php } else { ?>
              <div class="projectUserPermissions" id="user_<?php echo $user->getId() ?>_permissions">
                ...
                <div>
                  <?php echo checkbox_field('project_user_' . $user->getId() . '', $user->hasAllProjectPermissions(active_project()), array('id' => 'project_user_' . $user->getId() . '', 'class' => 'checkbox selectall' )) ?> <label for="<?php echo 'project_user_' . $user->getId() . '_all' ?>" class="checkbox" ><strong><?php echo lang('all permissions') ?><strong></label>
                </div>
